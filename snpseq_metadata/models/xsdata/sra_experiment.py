...
class LocusLocusName(Enum):
    ...
    :cvar VALUE_16_S_R_RNA: Bacterial small subunit ribosomal RNA, a
        locus used for                            phylogenetic studies
        of bacteria and as a target for random target PCR in
        environmental biodiversity screening.
    :cvar VALUE_18_S_R_RNA: Eukaryotic small subunit ribosomal RNA, a
        locus used for                            phylogenetic studies
        of eukaryotes and as a target for random target PCR in
        environmental biodiversity screening.
    :cvar VALUE_28_S_R_RNA: Structural ribosomal RNA for the large
        component, or large                             subunit (LSU) of
    ...
    :cvar ITS1_5_8_S_ITS2: Internal transcribed spacers 1 and 2 plus
        5.8S rRNA region,                            a locus used for
        phylogenetic studies of fungi.
...
class TypeLibrarySelection(Enum):
    ...
    :cvar PADLOCK_PROBES_CAPTURE_METHOD: Targeted sequence capture
        ...
        targets. An example is            capture bisulfite sequencing
        using padlock probes (BSPP).
...
class TypeLibraryStrategy(Enum):
    ...
    :cvar RNA_SEQ: Random sequencing of whole transcriptome, also known
        as Whole Transcriptome Shotgun Sequencing, or WTSS). (see
    ...
    :cvar NC_RNA_SEQ: Capture of other non-coding RNA types, including
        post-translation modification types such as snRNA (small
    ...
    :cvar AMPLICON: Sequencing of overlapping or distinct PCR or RT-PCR
        products. For example, metagenomic community profiling
        using SSU rRNA .
...
class LibraryDescriptorType:
    ...
    @dataclass
    class LibraryLayout:
        single: Optional["LibraryDescriptorType.LibraryLayout.Single"] = field(
            default=None,
            metadata={
                "name": "SINGLE",
                "type": "Element",
    ...
    class TargetedLoci:
        ...
        class Locus:
            """
            :ivar probe_set: Reference to an archived primer or
                probe set. Example: dbProbe
            :ivar locus_name:
            :ivar description: Submitter supplied description of
                alternate locus and auxiliary
                information.
            """
...
class SampleDescriptorType(RefObjectType):
    ...
    class Pool:
        ...
        :ivar default_member: Reference to the sample that is used when
            ...
            should                  be provided if there exists a
            possibility that some reads will be left over from
            barcode/MID resolution. A default member                  is
            not needed when defining a true pool (where individual
            samples are not distinguished in the reads), or the reads
            have                  been partitioned among the pool
...
class LibraryType:
    ...
    :ivar sample_descriptor: Pick a sample to associate this experiment
        with. The sample may be an individual or a pool,
        depending on how it is specified.
    :ivar library_descriptor: The LIBRARY_DESCRIPTOR specifies the
        origin of the material being sequenced and any
        treatments that the material might have undergone that affect
        the sequencing result. This specification is            needed
        even if the platform does not require a library construction
        step per se.
    :ivar spot_descriptor: The SPOT_DESCRIPTOR specifies how to decode
        the individual reads of interest from the            monolithic
        spot sequence. The spot descriptor contains aspects of the
        experimental design, platform, and            processing
        information. There will be two methods of specification: one
        will be an index into a table of            typical decodings,
        the other being an exact specification. This construct is needed
        for loading data and for            interpreting the loaded
        runs. It can be omitted if the loader can infer read layout
        (from multiple input            files or from one input files).
    """
...
class ExperimentType(ObjectType):
    ...
    study_ref: Optional["ExperimentType.StudyRef"] = field(
        default=None,
        metadata={
            "name": "STUDY_REF",
            "type": "Element",

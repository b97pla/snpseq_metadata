...
class ModelConversionException(MetadataException):
    def __init__(
        self, source: Type, target: Type, reason: Optional[Exception] = None
    ) -> None:
        reason = f", reason: {str(reason)}" if reason is not None else ""
        self.message = f"{source.__name__} could not be converted to a suitable {target.__name__} object{reason}"
